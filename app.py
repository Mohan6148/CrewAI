...
def build_gemini_llm(temperature):
    return LLM(
        model=GEMINI_MODEL,
        api_key=API_KEY,
...
try:
    planner_llm = build_gemini_llm(0.7)
    writer_llm = build_gemini_llm(0.3)
    reviewer_llm = build_gemini_llm(0.1)
...
def run_crew_async(user_prompt, task_id, attachments=None):
    ...
    try:
        ...
        if is_quota_error(e):
            ...
            crew_status["current_task"] = "Gemini quota exceeded"
            crew_status["error"] = (
                f"Gemini API quota error for model {GEMINI_MODEL}. "
                "Your Google AI Studio key is valid, but this model currently has no free-tier quota for your project. "
                "Try setting GEMINI_MODEL to a different Gemini flash model in .env, or enable billing on the Google project."
            )
            save_task_to_history(task_id, user_prompt, "error", error_message=crew_status["error"])
            return
